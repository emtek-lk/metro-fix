...
export interface AssignWorkerModalProps {
  ...
  jobTitle?: string;
  onClose: () => void;
  onWorkerAssigned: (workerName: string) => void;
}
...
export function AssignWorkerModal({
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/workers`, {
      ...
      .then((res) => {
        if (!res.ok) throw new Error('Failed to fetch field workers list');
        return res.json();
      })
      .then((data: any[]) => {
        setIsLoading(false);
        const mapped: WorkerOption[] = data.map((item) => ({
          id: item.id,
    ...
    <div style={styles.overlay} onClick={onClose}>
      ...
      >
        <div style={styles.header}>
          <div>
            <h2 id="assign-worker-title" style={styles.title}>Assign Worker to Service Ticket</h2>
            <p style={styles.subtitle}>
              Ticket: <strong style={{ color: '#f38808' }}>{jobTitle || jobId}</strong>
            </p>
          </div>
        ...
        {isLoading ? (
          <div style={styles.loadingState}>
            <span>🔄 Loading available field workers from database...</span>
          </div>
        ) : (
          <form onSubmit={handleSubmit} style={styles.form}>
            <div style={styles.fieldGroup}>
              <label style={styles.label} htmlFor="worker-select">
                Select Dispatch Worker *
              </label>
              <select
                id="worker-select"
              ...
              >
                {workers.map((w) => (
                  <option key={w.id} value={w.id}>
                    {w.fullName} (Rating: {w.rating} ★)
                    {w.servicePillars && w.servicePillars.length > 0
            ...
            <div style={styles.actions}>
              ...
              <button type="submit" disabled={isSubmitting} style={styles.submitBtn}>
                {isSubmitting ? 'Assigning...' : 'Dispatch Worker'}
              </button>
            </div>
          </form>
        )}
